...
function resetInitialState() {
    gameState.board = newBoard(),
        gameState.apple = [Math.floor(Math.random() * 12), Math.floor(Math.random() * 12)],
        gameState.snake = {
            body: [
                [10, 7],
                [10, 8],
...
function moveSnake() {
    ...
    scoreCount.innerText = 'Score: ' + gameState.score
...
function changePhaseTo(newPhase) {
    ...
    if (gameState.phase === PLAYING) {
        ...
        final.innerText = 'Your final score was: ' + gameState.score
        final.style.display = 'block'

        button.style.display = 'block';

        playAgainMessage = ["You can do better than that, wanna play again?", "That's all you got?", `Nice try, but maybe next time you can do better than ${gameState.score}`]
        paIndex = Math.floor(Math.random() * playAgainMessage.length)
        alert.innerText = playAgainMessage[paIndex];
...
// board.addEventListener('keyUp', turnSnake)
